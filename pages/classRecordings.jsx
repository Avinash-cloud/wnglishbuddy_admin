...
export default function classRecordingss() {
  ...
  useEffect(() => {
    const fetchclassRecordings = async () => {
      setLoading(true);
      try {
        const response = await axios.get('/api/classRecordings', {
          params: {
            page: currentPage,
            limit: rowsPerPage,
            search: searchTerm
        ...
        setclassRecordings(response.data.classRecordings); // Assuming the data contains a 'classRecordingss' array
      ...
      <div className="flex col-a justify-center border-e-red-50">
        <section className="mx-auto w-full  px-4 py-4">
          <div className="flex flex-col space-y-4 md:flex-row md:items-center md:justify-between md:space-y-0">
            <div>
              <h2 className="text-lg font-semibold">classRecordingss</h2>
              <p className="mt-1 text-sm text-gray-700">
                This is a list of all classRecordingss. You can add new classRecordingss, edit or delete existing
                ones.
              </p>
            </div>
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="divide-y divide-gray-200 bg-white">
                      {loading ? (
                        ...
                        classRecordings.map((classRecordings,index) => (
                          <tr key={index}>
                              ...
                              <div className="flex items-center">
                                <div className="h-10 w-10 flex-shrink-0">
                                  <img
                                    ...
                                    src={classRecordings.image}
                                    alt=""
                                  />
                                </div>
                                <div className="ml-4">
                                  <div className="text-sm font-medium text-gray-900">{classRecordings.name}</div>
                                  
                                </div>
                              </div>
                            </td>
                            <td className="whitespace-nowrap px-12 py-4">
                              <div className="text-sm text-gray-900">{classRecordings.teacher}</div>
                              {/* <div className="text-sm text-gray-700">{classRecordings.department}</div> */}
                            </td>
                            <td className="whitespace-nowrap px-4 py-4">
                              <span className="inline-flex rounded-full bg-green-100 px-2 text-xs font-semibold leading-5 text-green-800">
                                ₹ {classRecordings.price}
                              </span>
                            </td>
                            <td className="whitespace-nowrap px-4 py-4 text-sm text-gray-700">
                              {classRecordings.students_no}
                            </td>
                            <td className="whitespace-nowrap px-4 py-4 text-right text-sm font-medium">
                              <a href="/classRecordingss/details" className="text-gray-700">
                                <ViewIcon />
                              </a>
                            </td>
                          </tr>
